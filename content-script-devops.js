...
chrome.runtime.onMessage.addListener(function (request) {
    ...
    //     }, 1000);
    // } else 
    if (request && request.type === 'git-repos-fetched') {
        setTimeout(() => {
            addGitButtonToPopup();
        }, 200);
    }
...
function addGitButtonToPopup(){

    const branchNameSection = $('.vc-create-branch-from-git-ref-dialog .form-section:first-child');
    const fillBranchNameButton = $('<button style="margin-top:0.5rem" class="ui-button">use item id and name</button>');
    branchNameSection.append(fillBranchNameButton);

    fillBranchNameButton.click(_ => {
        const header = $('.work-item-form-main-header');
        if (!(header?.length > 0)){
            console.log('>> devops content script: header not found');
            return;
